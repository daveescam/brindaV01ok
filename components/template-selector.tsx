...
import { ChatToxico } from "./ai-templates/chat-toxico"
import { InstagramDespechado } from "./ai-templates/instagram-despechado"
import { MemeDespecho } from "./ai-templates/meme-despecho"
import { RevistaDrama } from "./ai-templates/revista-drama"
import { PerfilCitas } from "./ai-templates/perfil-citas"
import { NotificacionRedFlag } from "./ai-templates/notificacion-redflag"
...
export function TemplateSelector({ onSelectTemplate, initialTemplate = "chat_toxico" }: TemplateSelectorProps) {
  ...
  useEffect(() => {
    // Si hay un initialTemplate, seleccionarlo automáticamente
    if (initialTemplate) {
      const template = DESPECHO_TEMPLATES.find((t) => t.type === initialTemplate)
      if (template && onSelectTemplate) {
        onSelectTemplate(template)
      }
    }
  }, [initialTemplate, onSelectTemplate])
        ...
        <TabsContent value="all">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {DESPECHO_TEMPLATES.map((template) => (
              <Card
                key={template.id}
                className={`overflow-hidden transition-all ${selectedTemplate === template.type ? "ring-2 ring-pink-500" : ""}`}
              >
